...
contract ZodiacModuleTest is BasesTest {
    ...
    function testRawStorage() public {
        ...
        vm.mockCall(fakeGuard, abi.encodeWithSignature("supportsInterface(bytes4)"), abi.encode(true));
        ...
        assertEq(vm.load(address(module), bytes32(zodiacStateBaseSlot)).fromLast20Bytes(), fakeTarget);
        assertEq(vm.load(address(module), bytes32(zodiacStateBaseSlot + 1)).fromLast20Bytes(), fakeGuard);
        assertEq(bytes32(zodiacStateBaseSlot + 2), keccak256("firm.zodiacmodule.state"));
