...
abstract contract RolesAuth {
    ...
    function _isAuthorized(address _actor, address _authorizedAddr) internal view returns (bool) {
        if (_actor == _authorizedAddr) {
            return true;
        }
        ...
        return flag & ROLE_FLAG_MASK == 0x01 && roles.hasRole(_actor, uint8(flag >> 8));
    }
}
